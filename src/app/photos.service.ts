    ...
    return new Observable<Photo[]>((observer) => {
      this.auth.user.subscribe((user) => {
        user?.getIdToken().then((token) => {
          if (token) {
            this.http
              .get<Photo[]>('/api/my-photos', httpOptionsWithAuthToken(token))
    ...
    return new Observable<Photo[]>((observer) => {
      this.auth.user.subscribe((user) => {
        user?.getIdToken().then((token) => {
          if (token) {
            this.http
              .get<Photo[]>('/api/shared', httpOptionsWithAuthToken(token))
              .subscribe((photos) => observer.next(photos));
          }
        });
      });
          ...
          if (token) {
            this.http
              .post<void>(
                '/api/upload',
                formData,
                httpOptionsWithAuthToken(token, true)
              )
      ...
      this.auth.user.subscribe((user) => {
        user?.getIdToken().then((token) => {
          if (token) {
            this.http
              .post<Photo>(
                `/api/photos/${photoId}/shared-with`,
                { email },
                httpOptionsWithAuthToken(token)
              )
              .subscribe((photo) => observer.next(photo));
